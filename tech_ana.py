...
def signal_2(df: pd.DataFrame, rsi_length: int = 14, arn_length: int = 25, reversal_check_length:int=40):
    # calculate indicator
    df['rsi'] = ta.momentum.rsi(df['close'], window=rsi_length)
...
def signal_3(df: pd.DataFrame, rsi_length: int = 14, arn_length: int = 25, reversal_check_length: int = 40):
    # calculate indicator
    df['rsi'] = ta.momentum.rsi(df['close'], window=rsi_length)
    vt = ta.trend.VortexIndicator(df['high'], df['low'], df['close'],window=20)
